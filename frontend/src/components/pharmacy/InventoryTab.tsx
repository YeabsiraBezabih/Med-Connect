...
import pharmacyService, { Medicine, CreateMedicineData } from '../../services/pharmacy.service';
...
const InventoryTab = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!pharmacyId) {
      ...
      showToast('Pharmacy profile not found. Please contact support or re-register as a pharmacy.', 'error');
      return;
    }
    ...
    if (currentMedicine) {
      ...
      updatedMedicine = await pharmacyService.updateMedicine(currentMedicine.id, medicineData);
      setInventory(prev => prev.map(item => item.id === currentMedicine.id ? updatedMedicine : item));
      ...
      try {
        ...
        if (typeof error === 'object' && error && 'response' in error && (error as { response?: { status?: number } }).response?.status === 400) {
          const err = error as { response?: { data?: { pharmacy?: string[]; message?: string } } };
          const errorMessage = err.response?.data?.pharmacy?.[0] || 
  ...
  const handleDelete = async (medicineId: number) => {
    if (window.confirm('Are you sure you want to delete this medicine?')) {
      try {
        ...
        setInventory(prev => prev.filter(medicine => medicine.id !== medicineId));
              ...
              {filteredInventory.map((medicine) => (
                <tr key={medicine.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {typeof medicine.price === 'number' ? medicine.price.toFixed(2) :
                      !isNaN(parseFloat(medicine.price)) ? parseFloat(medicine.price).toFixed(2) : 'N/A'}
                  </td>
